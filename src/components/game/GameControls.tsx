...
export const GameControls: React.FC = () => {
    const { undo, getHint, addRows, history, grid, isPaused, togglePause, highlightAddRows, addsCount } = useGameStore();

    // Limit check
    const MAX_ADDS = 10;
    const remainingAdds = Math.max(0, MAX_ADDS - addsCount);
    const canAddRows = !isPaused && remainingAdds > 0;
    ...
    return (
        <div className="flex items-center justify-center gap-4 py-4 px-2">
            ...
            <ControlButton
                ...
                disabled={!canAddRows}
                icon={<Plus size={20} />}
                label={`Más filas (${remainingAdds})`}
                className={clsx(highlightAddRows && "animate-bounce ring-4 ring-pink-500/50 rounded-full")}
            />
